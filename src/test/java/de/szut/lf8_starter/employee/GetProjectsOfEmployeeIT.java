...
public class GetProjectsOfEmployeeIT extends AbstractIntegrationTest {

    @Test
    @WithMockUser(roles = "user")
    void getProjectsOfEmployee() throws Exception {
        ...
        final var projectsOfEmployeeContentAsString = this.mockMvc.perform(get("/employee/1/projects")
                        .with(csrf()))
                .andExpect(status().is2xxSuccessful())
                .andExpect(jsonPath("$[0].id").exists())
                .andExpect(jsonPath("$[0].name", is("New Website Development")))
                .andExpect(jsonPath("$[0].customer.id", is(1)))
                .andExpect(jsonPath("$[0].projectManager.id", is(1)))
                .andExpect(jsonPath("$[0].startDate", is("2025-01-15")))
                .andExpect(jsonPath("$[0].endDate", is("2025-06-30")))
                .andReturn()
                .getResponse()
                .getContentAsString();
    }
