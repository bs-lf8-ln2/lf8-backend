...
public class ProjectDeleteIT extends AbstractIntegrationTest {
    ...
    void testDeleteProject() throws Exception {
        ...
        this.mockMvc.perform(post("/qualifications").content(qualificationJson).contentType(MediaType.APPLICATION_JSON)
                        .with(csrf()))
        ...
        this.mockMvc.perform(post("/customer").content(customerJson).contentType(MediaType.APPLICATION_JSON)
                        .with(csrf()))
                .andExpect(status().is2xxSuccessful())
                .andExpect(jsonPath("id").exists())
        ...
        final var loadedEntityAfterDelete = projectRepository.findById(projectId);
        assertThat(loadedEntityAfterDelete).isEmpty();

        // get 404 when trying to delete the same project again
        this.mockMvc.perform(delete("/projects/" + projectId)
                        .with(csrf()))
